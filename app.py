...
def calculate_macd_manual(series, fast=MACD_FAST, slow=MACD_SLOW, signal=MACD_SIGNAL):
    ...
    series = series.dropna()
    if len(series) < slow + signal : return np.nan, np.nan, np.nan
...
def calculate_vwap_manual(df, period=VWAP_PERIOD):
    # Assicura che le colonne necessarie esistano e non siano tutte NaN
    if not all(col in df.columns for col in ['close', 'volume']) or \
       df['close'].isna().all() or df['volume'].isna().all():
        return np.nan
    
    df_period = df.iloc[-period:].copy() # Lavora su una copia dell'ultimo periodo
    
    # Pulisci NaN nel periodo specifico
    df_period.dropna(subset=['close', 'volume'], inplace=True)
    if df_period.empty or len(df_period) == 0 or df_period['volume'].sum() == 0:
         return np.nan # Non calcolabile se non ci sono dati o volumi nel periodo

    # Calcola VWAP: Sum(Price * Volume) / Sum(Volume)
    vwap = (df_period['close'] * df_period['volume']).sum() / df_period['volume'].sum()
    return vwap

# --- Funzione Segnale GPT Semplificato (Invariata) ---
def generate_signal(rsi_val, macd_hist_val, ma_short_val, ma_long_val):
    if pd.isna(rsi_val) or pd.isna(macd_hist_val) or pd.isna(ma_short_val) or pd.isna(ma_long_val): return "⚪️ N/D"
    score = 0
    if rsi_val < 30: score += 2; elif rsi_val < 40: score += 1; elif rsi_val > 70: score -= 2; elif rsi_val > 60: score -= 1
    if macd_hist_val > 0: score += 1; else: score -= 1
    if ma_short_val > ma_long_val: score += 1; else: score -= 1
    if score >= 3: return "🔶 Strong Buy"; elif score >= 1: return "🟢 Buy"; elif score <= -3: return "🔻 Strong Sell"; elif score <= -1: return "🔴 Sell"; else: return "🟡 Hold"
...
for i, coin_id in enumerate(COINGECKO_IDS_LIST):
    ...
    if not hist_df.empty:
        # Calcola indicatori manualmente
        ...
        vwap = calculate_vwap_manual(hist_df) # Passa intero df per volume

        # Verifica se qualche calcolo ha fallito per mancanza di dati nel df storico
        if pd.isna(rsi1d) and len(hist_df) < RSI_PERIOD + 1 : fetch_errors.append(f"Dati insuff. ({len(hist_df)}) per RSI per {symbol}.")
        # Aggiungi controlli simili per MACD, MA se necessario basati sulla lunghezza minima
    else:
...
if results:
    ...
    df['GPT Signal'] = df.apply(lambda row: generate_signal(row['RSI (1d)'], row['MACD Hist (1d)'], row[f'MA({MA_SHORT}d)'], row[f'MA({MA_LONG}d)']), axis=1)
