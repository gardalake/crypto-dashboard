# Versione: v14 - Riattivato AV (opz.'d'), Aggiornata lista crypto (AERGO in, TRUMP fix, FART in, +31 totali), Rimossa colonna RSI(1h), Aggiunta colonna Link CG.
# -*- coding: utf-8 -*-
...
# --- AGGIORNATO MAPPA SIMBOLI ---
SYMBOL_TO_ID_MAP = {
    "BTC": "bitcoin", "ETH": "ethereum", "BNB": "binancecoin", "SOL": "solana", "XRP": "ripple",
    "RNDR": "render-token", "FET": "fetch-ai", # VERIFICARE ID!
    ...
    "PEPE": "pepe", "UNI": "uniswap", "TIA": "celestia", "JUP": "jupiter-aggregator", # VERIFICARE ID!
    "IMX": "immutable-x",
    "TRUMP": "official-trump", # CORRETTO ID come da link utente
    "NEAR": "near", # VERIFICARE ID!
    "AERO": "aerodrome-finance", "TRON": "tron",
    "AERGO": "aergo", # RIAGGIUNTO AERGO
    "ADA": "cardano", "MKR": "maker",
    "WLD": "worldcoin-org", # ID Confermato
    "HYPE": "hyperliquid", # ID Confermato
    "FART": "fartcoin" # AGGIUNTO FARTCOIN
}
SYMBOLS = list(SYMBOL_TO_ID_MAP.keys())
COINGECKO_IDS_LIST = list(SYMBOL_TO_ID_MAP.values())
NUM_COINS = len(SYMBOLS) # Ora dovrebbe essere 31
...
def get_coingecko_historical_data(coin_id, currency, days, interval='daily'):
    ...
    try:
        ...
        elif status_code == 404: status_msg = f"Not Found (404) ({coin_id})" # ID errato!
...
def get_traditional_market_data_av(tickers):
    ...
    for ticker_sym in tickers:
        # Limite giornaliero: nota che questo contatore si resetta ad ogni esecuzione della funzione se non è nella cache
        # La cache di 4h aiuta, ma 25 chiamate/giorno è il limite vero
        # Non c'è modo semplice di tracciare il limite giornaliero tra esecuzioni diverse qui
        # if calls_made >= 25: ... (commentato perché inefficace senza stato persistente)
        ...
        try:
            ...
            if not quote_data.empty:
             ...
             # Interrompi solo se l'errore è chiaramente legato a limiti/chiave
             if "call frequency" in str(ve).lower() or "api key" in str(ve).lower() or "limit" in str(ve).lower() or "premium" in str(ve).lower():
                 logger.error("Errore chiave/limite API Alpha Vantage rilevato. Interruzione fetch."); st.error("Errore chiave/limite API Alpha Vantage rilevato."); break
...
def compute_all_indicators(symbol, hist_daily_df, hist_hourly_df):
    indicators = {"RSI (1d)": np.nan, "RSI (1w)": np.nan, "RSI (1mo)": np.nan,"SRSI %K (1d)": np.nan, "SRSI %D (1d)": np.nan,"MACD Line (1d)": np.nan, "MACD Signal (1d)": np.nan, "MACD Hist (1d)": np.nan,f"MA({MA_SHORT}d)": np.nan, f"MA({MA_LONG}d)": np.nan,"VWAP (1d)": np.nan} # RSI(1h) rimosso
...
def generate_gpt_signal(rsi_1d, rsi_1h, rsi_1w, macd_hist, ma_short, ma_long, srsi_k, srsi_d, current_price):
    # rsi_1h non viene più calcolato, impostiamolo a NaN per sicurezza se usato nel punteggio
    rsi_1h = np.nan # Assicura che non influenzi il punteggio se non disponibile
...
try: # Blocco try principale
    ...
    # RIGA 1 Overview
    overview_items_row1 = [("Fear & Greed Index", None, get_fear_greed_index, "Fonte: Alternative.me"),(f"Total Crypto M.Cap ({VS_CURRENCY.upper()})", None, lambda: f"${format_large_number(total_market_cap)}", "Fonte: CoinGecko"),("Crypto ETFs Flow (Daily)", None, get_etf_flow, "Dato N/A"),("S&P 500 (SPY)", "SPY", None, "Fonte: Alpha Vantage (via ETF)"),("Nasdaq (QQQ)", "QQQ", None, "Fonte: Alpha Vantage (via ETF)")]
    ...
    # RIGA 2 Overview
    overview_items_row2 = [("Gold (GLD)", "GLD", None, "Fonte: Alpha Vantage (via ETF)"), ("Silver (SLV)", "SLV", None, "Fonte: Alpha Vantage (via ETF)"), ("Natural Gas (UNG)", "UNG", None, "Fonte: Alpha Vantage (via ETF)"), ("UVXY (Volatility)", "UVXY", None, "Fonte: Alpha Vantage"), ("TQQQ (Nasdaq 3x)", "TQQQ", None, "Fonte: Alpha Vantage")]
    ...
    with st.spinner(f"Recupero dati storici e calcolo indicatori per {NUM_COINS} crypto... (Richiede ~{estimated_wait_mins:.1f} min)"):
        ...
        for i, coin_id in enumerate(coin_ids_ordered):
            symbol = next((sym for sym, c_id in SYMBOL_TO_ID_MAP.items() if c_id == coin_id), "N/A") # Trova simbolo da ID
            logger.info(f"--- Elaborazione {i+1}/{len(coin_ids_ordered)}: {symbol} ({coin_id}) ---")

            try: # Try per ciclo singolo coin
                if symbol == "N/A": msg = f"{coin_id}: ID non trovato nella mappa SYMBOL_TO_ID_MAP."; logger.warning(msg); fetch_errors_for_display.append(msg); continue # Skip if ID not in our map
                ...
                # Aggiungi il link CoinGecko ai risultati
                ...
                results.append({
                    ...
                    "Link": coingecko_link # Aggiunta colonna Link
                })
                ...
                st.warning(f"Errore elaborazione {symbol}. Controlla log sotto."); fetch_errors_for_display.append(f"{symbol}: Errore Elaborazione - Vedi Log")
    ...
    if results:
        ...
        try:
            ...
            # Rimosso RSI(1h) da rsi_srsi_cols se esisteva
            rsi_srsi_cols = [col for col in df_display.columns if ("RSI" in col or "SRSI" in col) and col != "RSI (1h)"];
            macd_cols = [col for col in df_display.columns if "MACD" in col]; ma_vwap_cols = [col for col in df_display.columns if "MA" in col or "VWAP" in col]
            ...
            st.dataframe(
                ...
                column_config={
                    "Link": st.column_config.LinkColumn(
                        ...
                        display_text="🔗 Link" # Testo visualizzato nella cella
                    )
                }
            )
