...
try:
    ...
    st.warning("Modulo 'zoneinfo' non trovato. Usando fallback UTC+2 per Roma.")
    ZoneInfo = None
...
SYMBOL_TO_ID_MAP = {
    ...
    "TRUMP": "maga", # ID 'maga' ha ticker TRUMP su CoinGecko. Verificare.
    "NEAR": "near", "AERO": "aerodrome-finance", "TRON": "tron", "AERGO": "aergo", "ADA": "cardano",
    "MKR": "maker", "WLD": "worldcoin-org", "HYPE": "hyperliquid"
}
...
def get_coingecko_historical_data(coin_id, currency, days, interval='daily'):
    # [Invariata con logging e pausa 4s]
...
try: # Blocco try principale
    ...
    for i, (label, ticker, func, help_text) in enumerate(overview_items_row1):
        with overview_cols_1[i]:
            value_str = "N/A"; delta_txt = None; d_color = "off"; change = np.nan # Initialize defaults
            if ticker:
                ...
                if pd.notna(change): d_color = "normal" # Set color only if change is valid
            elif func: value_str = func() # Calculate value for non-ticker items
            st.metric(label=label, value=value_str, delta=delta_txt, delta_color=d_color, help=help_text)

    # RIGA 2 Overview (5 colonne)
    ...
    # SEZIONE TITOLI PRINCIPALI (5 colonne, 2 righe)
    ...
    # --- CICLO PROCESSING PER OGNI COIN (REINTRODOTTO) ---
    results = []; fetch_errors_for_display = [] # Lista separata per UI
    process_start_time = time.time()
    logger.info(f"Inizio ciclo elaborazione per {NUM_COINS} crypto.")
    estimated_wait_secs = NUM_COINS * 2 * 4 # Stima approssimativa
    ...
    with st.spinner(f"Recupero dati storici e calcolo indicatori per {NUM_COINS} crypto... (Richiede ~{estimated_wait_mins:.1f} min)"):
        ...
        for i, coin_id in enumerate(coin_ids_ordered):
            ...
            try: # Try per ciclo singolo coin
                if coin_id not in market_data_df.index: msg = f"{coin_id}: Dati live non trovati."; logger.warning(msg); fetch_errors_for_display.append(msg); continue
                ...
                if status_daily != "Success": fetch_errors_for_display.append(f"{symbol}: Daily - {status_daily}") # Usa logger interno alla funzione
                hist_hourly_df, status_hourly = get_coingecko_historical_data(coin_id, VS_CURRENCY, DAYS_HISTORY_HOURLY, interval='hourly')
                if status_hourly != "Success": fetch_errors_for_display.append(f"{symbol}: Hourly - {status_hourly}") # Usa logger interno alla funzione
                # Calcola indicatori (logga errori interni)
                ...
                results.append({
                    ...
                    "RSI (1h)": indicators.get("RSI (1h)"), "RSI (1d)": indicators.get("RSI (1d)"), "RSI (1w)": indicators.get("RSI (1w)"), "RSI (1mo)": indicators.get("RSI (1mo)"),
    ...
    with st.expander("📘 Legenda Indicatori Tecnici e Segnali", expanded=False):
        ...
        * **S&P 500 (SPY), Nasdaq (QQQ), Gold (GLD), etc.:** Prezzi indicativi mercati tradizionali (Fonte: Alpha Vantage). **Aggiornati con ritardo (cache lunga 4h)** causa limiti API gratuite. Visualizza variazione giornaliera (Rosso/Verde).
        * **Titoli Principali:** Prezzi indicativi azioni (Fonte: Alpha Vantage). **Aggiornati con ritardo (cache lunga 4h).** Visualizza variazione giornaliera (Rosso/Verde).
