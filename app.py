...
import pandas as pd
import pandas_ta as ta # <--- Added pandas-ta
import requests
import numpy as np
from datetime import datetime, timedelta
import time
import plotly.graph_objects as go # <--- Added Plotly graph objects
from plotly.subplots import make_subplots # <--- Added Plotly subplots
# import math
...
# Use a different cache function for chart data to allow separate TTL
@st.cache_data(ttl=CACHE_CHART_TTL, show_spinner=False)
def get_coingecko_historical_data_for_chart(coin_id, currency, days):
    """Recupera i dati storici da CoinGecko per il grafico (daily)."""
    logger.debug(f"CHART: Inizio fetch storico per {coin_id} (daily, {days}d).")
    # No sleep needed here usually as it's only called once per selection
    url = f"https://api.coingecko.com/api/v3/coins/{coin_id}/market_chart"
    params = {
        'vs_currency': currency, 'days': str(days), 'interval': 'daily', 'precision': 'full'
    }
    status_msg = f"Errore Sconosciuto ({coin_id}, daily chart)"
    try:
        logger.debug(f"CHART: Requesting URL: {url} with params: {params}")
        response = requests.get(url, params=params, timeout=25)
        response.raise_for_status()
        data = response.json()
        if not data or 'prices' not in data or not data['prices']:
            status_msg = f"No Prices Data ({coin_id}, daily chart)"
        ...
        # Create approximate OHLC if not available (CoinGecko daily doesn't provide it)
        ...
        if hist_df.empty:
            status_msg = f"Processed Empty ({coin_id}, daily chart)"
            logger.warning(status_msg)
            return pd.DataFrame(), status_msg
        ...
        logger.warning(f"CHART: Errore HTTP API Storico CoinGecko: {status_msg}")
        return pd.DataFrame(), status_msg
    except requests.exceptions.RequestException as req_ex:
        status_msg = f"Request Error ({req_ex}) ({coin_id}, daily chart)"
        logger.error(f"CHART: Errore Richiesta API Storico CoinGecko: {status_msg}")
        return pd.DataFrame(), status_msg
    except Exception as e:
        status_msg = f"Generic Error ({type(e).__name__}) ({coin_id}, daily chart)"
        logger.exception(f"CHART: Errore Processamento API Storico CoinGecko per {coin_id}:")
        return pd.DataFrame(), status_msg


# Use the regular cache for indicator calculation data fetch
@st.cache_data(ttl=CACHE_HIST_TTL, show_spinner=False)
def get_coingecko_historical_data(coin_id, currency, days, interval='daily'):
    """Recupera i dati storici da CoinGecko con pausa per rate limiting (per indicatori tabella)."""
    logger.debug(f"TABLE: Inizio fetch storico per {coin_id} ({interval}), pausa 6s...")
    time.sleep(6.0)
    logger.debug(f"TABLE: Fine pausa per {coin_id} ({interval}), inizio chiamata API.")

    url = f"https://api.coingecko.com/api/v3/coins/{coin_id}/market_chart"
    params = {
        'vs_currency': currency, 'days': str(days),
        'interval': interval if interval == 'hourly' else 'daily',
        'precision': 'full'
    }
    status_msg = f"Errore Sconosciuto ({coin_id}, {interval})"
    try:
        logger.debug(f"TABLE: Requesting URL: {url} with params: {params}")
        response = requests.get(url, params=params, timeout=25)
        response.raise_for_status()
        data = response.json()
        if not data or 'prices' not in data or not data['prices']:
        ...
        hist_df = hist_df.interpolate(method='time').ffill().bfill()
        # Approximation needed for manual indicators if OHLC not present
        ...
        status_msg = "Success"
        logger.info(f"TABLE: Dati storici CoinGecko per {coin_id} ({interval}) recuperati, {len(hist_df)} righe.")
        return hist_df, status_msg # Return the full df needed for indicators
        ...
        logger.warning(f"TABLE: Errore HTTP API Storico CoinGecko: {status_msg}")
        return pd.DataFrame(), status_msg
    except requests.exceptions.RequestException as req_ex:
        status_msg = f"Request Error ({req_ex}) ({coin_id}, {interval})"
        logger.error(f"TABLE: Errore Richiesta API Storico CoinGecko: {status_msg}")
        return pd.DataFrame(), status_msg
    except Exception as e:
        status_msg = f"Generic Error ({type(e).__name__}) ({coin_id}, {interval})"
        logger.exception(f"TABLE: Errore Processamento API Storico CoinGecko per {coin_id} ({interval}):")
        return pd.DataFrame(), status_msg
...
def get_global_market_data_cg(currency):
    """Recupera i dati globali da CoinGecko."""
...
def get_traditional_market_data_av(tickers):
    """Recupera dati quote da Alpha Vantage."""
    ...
    try:
        ...
        st.error("Errore Configurazione: Chiave API Alpha Vantage non trovata.")
        return data
    except Exception as e:
    ...
    if not api_key:
        logger.error("Chiave API Alpha Vantage vuota.")
        st.error("Errore Configurazione: Chiave API Alpha Vantage vuota.")
        return data

    ts = TimeSeries(key=api_key, output_format='pandas')
    calls_made = 0
    max_calls_per_minute = 5
    max_calls_this_run = 25
    delay_between_calls = (60.0 / max_calls_per_minute) + 1.0

    for ticker_sym in tickers:
        if calls_made >= max_calls_this_run:
            msg = f"Limite chiamate AV ({max_calls_this_run}) raggiunto. Stop fetch per {ticker_sym}."
            logger.warning(msg)
            st.warning(msg)
            break
        ...
        try:
            ...
            if not quote_data.empty:
                ...
                except (KeyError, ValueError, IndexError, TypeError): pass # Lascia NaN
                ...
                except (KeyError, IndexError, TypeError): pass # Lascia 'N/A'
             ...
             if "call frequency" in ve_str or "api key" in ve_str or "limit" in ve_str or "premium" in ve_str:
                 logger.error("Errore chiave/limite API AV. Interruzione fetch.")
                 st.error("Errore chiave/limite API AV. Interruzione fetch.")
                 break
             data[ticker_sym] = {'price': np.nan, 'change': np.nan, 'change_percent': 'N/A'}
        except Exception as e:
             msg = f"Errore generico fetch AV per {ticker_sym}: {e}"
             logger.exception(msg)
             st.warning(msg)
             data[ticker_sym] = {'price': np.nan, 'change': np.nan, 'change_percent': 'N/A'}
...
def compute_all_indicators(symbol: str, hist_daily_df: pd.DataFrame) -> dict:
    ...
    if not hist_daily_df.empty and 'close' in hist_daily_df.columns:
        ...
        if len_daily >= min_len_srsi_base:
            ...
            indicators["SRSI %K (1d)"] = k; indicators["SRSI %D (1d)"] = d
...
def generate_gemini_alert(ma_short, ma_long, macd_hist, rsi_1d, vwap_1d, current_price):
    ...
    is_not_extremely_overbought = rsi_1d < 80; is_not_extremely_oversold = rsi_1d > 20
...
# --- Funzione Creazione Grafico (Nuova v0.4) ---
...
try:
    ...
    def render_metric(column, label, value_func=None, ticker=None, data_dict=None, help_text=None):
        ...
        if ticker and data_dict:
            ...
            price = trad_info.get('price', np.nan); change = trad_info.get('change', np.nan)
    ...
    with st.expander("📘 Legenda Indicatori Tecnici e Segnali", expanded=False):
        ...
        **(Vedi sezioni precedenti per Market Overview e Tabella Analisi Tecnica)**

        **Grafico Dettaglio Coin:**
        *   **Candlestick:** Rappresentazione standard del prezzo (Apertura, Massimo, Minimo, Chiusura) per ogni giorno. Verde se Chiusura > Apertura, Rosso altrimenti. *Nota: OHLC sono approssimati da CoinGecko Daily.*
        *   **MA(20d) / MA(50d):** Medie Mobili Semplici calcolate sull'intero periodo visualizzato.
        *   **RSI (14d):** Relative Strength Index calcolato sull'intero periodo, mostrato nel pannello inferiore. Le linee tratteggiate indicano i livelli 70 (ipercomprato) e 30 (ipervenduto).

        **Note Importanti:**
        *   Il recupero dei dati storici per la **tabella** da CoinGecko è **rallentato** (pausa 6s/coin). Il caricamento iniziale può richiedere minuti.
        *   Il recupero dei dati storici per il **grafico** (solo per la coin selezionata) usa una cache separata e più breve e **non** ha la pausa di 6s, quindi dovrebbe essere più veloce dopo la selezione.
        *   Dati Mercati Tradizionali (Alpha Vantage) hanno **cache 4h** (limiti API free).
        *   **DYOR (Do Your Own Research):** Fai sempre le tue ricerche. Performance passate non garantiscono risultati futuri.
        """)

    st.divider()
    st.caption("Disclaimer: Strumento informativo/didattico. Non consulenza finanziaria. DYOR.")

except Exception as main_exception:
    logger.exception("!!! ERRORE NON GESTITO NELL'ESECUZIONE PRINCIPALE !!!")
