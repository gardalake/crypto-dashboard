# Version: v1.4.6 - Fix duplicate key error for debug checkboxes
# -*- coding: utf-8 -*-
...
logger.info("Logging configured for UI (v1.4.6).")
...
def get_coingecko_market_data(ids_list, currency): # Unchanged from v1.4.5
...
def get_coingecko_historical_data(coin_id, currency, days, interval='daily'): # Unchanged from v1.4.5
    ...
    try:
        ...
        status_code = http_err.response.status_code; status_msg = f"HTTP Err {status_code} ({coin_id}, {interval})"
        if status_code == 429: status_msg = f"Rate Limited (429) ({coin_id}, {interval})"
        logger.warning(f"TABLE Hist: CG API Err: {status_msg} for URL {url} with params {params}"); return pd.DataFrame(), status_msg 
...
def get_fear_greed_index(): # Unchanged from v1.4.5
    ...
    except requests.exceptions.RequestException as req_ex: status_code = req_ex.response.status_code if req_ex.response is not None else "N/A"; msg = f"API Error F&G Index (Status: {status_code}): {req_ex}"; logger.warning(msg); st.sidebar.warning(f"F&G Index API: {status_code}"); return "N/A" 
    except Exception as e: msg = f"Error Processing F&G Index: {e}"; logger.exception(msg); st.sidebar.warning(f"F&G Index: {e}"); return "N/A"

@st.cache_data(ttl=CACHE_TTL, show_spinner=False)
def get_global_market_data_cg(currency): # Unchanged from v1.4.5
    ...
    try:
        ...
        return total_mcap
    except requests.exceptions.RequestException as req_ex: msg = f"Global CoinGecko API Error: {req_ex}"; logger.warning(msg); st.sidebar.warning(f"Global MCap API: {req_ex}"); return np.nan
    except Exception as e: msg = f"Error Processing Global CoinGecko Data: {e}"; logger.exception(msg); st.sidebar.warning(f"Global MCap: {e}"); return np.nan
...
def get_traditional_market_data_av(tickers): # Unchanged from v1.4.5
    ...
    ts = TimeSeries(key=api_key, output_format='pandas'); calls_made = 0; max_calls_per_minute = 5; max_calls_this_run = 25; delay_between_calls = (60.0 / max_calls_per_minute) + 1.0 # Max 5 calls/min for free tier usually
...
def calculate_stoch_rsi(series: pd.Series, rsi_period: int = RSI_PERIOD, stoch_period: int = SRSI_PERIOD, k_smooth: int = SRSI_K, d_smooth: int = SRSI_D) -> tuple[float, float]: # Unchanged
    ...
    if rs_series.isna().all(): return np.nan, np.nan 
...
def compute_all_indicators(symbol: str, hist_daily_df: pd.DataFrame) -> dict: # Unchanged
    ...
    if len_daily >= min_len_bb: indicators["BB %B"], indicators["BB Width"], indicators["BB Width %Chg"] = bb_results[3], bb_results[4], bb_results[5]
...
def generate_gpt_signal(rsi_1d, rsi_1w, macd_hist, ma_short_val, ma_medium_val, ma_long_val, srsi_k, srsi_d, bb_pct_b, bb_width_chg, vwap_1d, current_price): # Unchanged
    ...
    if score >= 6.0: return "⚡️ Strong Buy"; elif score >= 3.0: return "🟢 Buy"; elif score <= -6.0: return "🚨 Strong Sell"; elif score <= -3.0: return "🔴 Sell"; elif score > 0: return "⏳ CTB"; elif score < 0: return "⚠️ CTS"; else: return "🟡 Hold"

def generate_gemini_alert(ma_medium_val, ma_long_val, macd_hist, rsi_1d, vwap_1d, current_price): # Unchanged
    try: current_price_num = float(current_price) if pd.notna(current_price) else np.nan
    except: current_price_num = np.nan
...
try:
    # Title, Refresh, Timestamp - Unchanged from v1.4.5
    col_title, _, col_button = st.columns([4, 1, 1]); with col_title: st.title("📈 Crypto Technical Dashboard Pro")
    ...
    # Market Overview Section - Unchanged from v1.4.5
    ...
    for idx, ticker_sym in enumerate(stock_tickers_row_av): render_metric(stock_cols[idx % num_stock_cols], label=ticker_sym, ticker=ticker_sym, data_dict=traditional_market_data, help_text=f"Ticker: {ticker_sym}") # Changed ticker to ticker_sym
    st.markdown("---")

    # Main Crypto Technical Analysis Table - Timestamp and Data Fetch unchanged
    ...
    if last_cg_update_utc:
        ...
        except Exception as e: logger.exception("Error formatting/converting timestamp:"); timestamp_display_str = f"*Timestamp conversion error. UTC: {last_cg_update_utc.strftime('%Y-%m-%d %H:%M:%S %Z') if last_cg_update_utc and last_cg_update_utc.tzinfo else str(last_cg_update_utc)}*"
    ...
    if market_data_df.empty:
        ...
        with st.spinner(spinner_msg): # Data processing loop unchanged from v1.4.5
            for i, coin_id in enumerate(market_data_df.index):
                symbol = next((s for s, cid in SYMBOL_TO_ID_MAP.items() if cid == coin_id), "N/A"); logger.info(f"--- Proc Table {i+1}/{effective_num_coins}: {symbol} ({coin_id}) ---")
                if symbol == "N/A": fetch_errors.append(f"{coin_id}: ID not in SYMBOL_TO_ID_MAP."); continue
                live_coin_data = market_data_df.loc[coin_id]; hist_df, hist_status = get_coingecko_historical_data(coin_id, VS_CURRENCY, DAYS_HISTORY_DAILY)
                current_price_val = pd.to_numeric(live_coin_data.get('current_price'), errors='coerce')
                if hist_status != "Success": fetch_errors.append(f"{symbol}: Hist.Data - {hist_status}"); indicators = compute_all_indicators(symbol, pd.DataFrame())
                else: indicators = compute_all_indicators(symbol, hist_df)
                ...
                results.append({ "Rank": live_coin_data.get('market_cap_rank', np.nan), "Symbol": symbol, "Name": live_coin_data.get('name', coin_id), "MA/MACD Cross Alert": gemini_alert_val, "Composite Score": gpt_signal_val, f"Price ({VS_CURRENCY.upper()})": current_price_val, "% 1h": live_coin_data.get('price_change_percentage_1h_in_currency', np.nan), "% 24h": live_coin_data.get('price_change_percentage_24h_in_currency', np.nan), "% 7d": live_coin_data.get('price_change_percentage_7d_in_currency', np.nan), "% 30d": live_coin_data.get('price_change_percentage_30d_in_currency', np.nan), "% 1y": live_coin_data.get('price_change_percentage_1y_in_currency', np.nan), **indicators, f"Volume 24h ({VS_CURRENCY.upper()})": live_coin_data.get('total_volume', np.nan), "Link": f"https://www.coingecko.com/en/coins/{coin_id}"})
            logger.info(f"Table processing loop done. Time: {time.time() - process_start_time:.1f}s")
        if fetch_errors: st.sidebar.error("Data Fetch/Processing Issues:"); 
        ...
        if results:
            try:
                ...
                # --- FIX: Unique keys for debug checkboxes ---
                debug_pre = st.checkbox("Show Pre-coercion dtypes", False, key="debug_pre_coercion_log_v146")
                if debug_pre: logger.info("--- Pre-coercion dtypes for df_display ---"); logger.info(df_display.dtypes)
                ...
                debug_post = st.checkbox("Show Post-coercion dtypes", False, key="debug_post_coercion_log_v146")
                if debug_post: logger.info("--- Post-coercion dtypes for df_display ---"); logger.info(df_display.dtypes)
                # --- END FIX ---

                def safe_formatter(fmt_str, col_name="<unknown>"): # Unchanged from v1.4.5
                    def _format(x):
                        if pd.isna(x): return "N/A"
                        if not isinstance(x, (int, float, np.number)): logger.warning(f"SafeFormatter: Non-numeric value '{x}' (type: {type(x)}) in col '{col_name}' for fmt '{fmt_str}'. Returning N/A."); return "N/A" 
                        try: return fmt_str.format(x)
                        except (ValueError, TypeError) as e: logger.warning(f"SafeFormatter: Format err for val '{x}' (type: {type(x)}) in col '{col_name}' with fmt '{fmt_str}'. Err: {e}. Ret str."); return str(x) 
                    return _format
                ...
                def format_1h_with_icon(val):
                    ...
                    if not isinstance(val, (int, float, np.number)): return "N/A"
                ...
                ma_vwap_cols = [col for col in ma_value_cols_pattern if col in df_display.columns] # Corrected in v1.4.5
                for col in ma_vwap_cols: formatters[col] = safe_formatter("{:,.2f}", col)
                
                # Styling functions (unchanged from v1.4.5)
                ...
                styled_table = styled_table.format(formatters, na_rep="N/A") # Apply formatting LAST
    ...
    # Legend and Disclaimer (Unchanged from v1.4.5)
    st.divider();
    with st.expander("📘 Indicator, Signal & Legend Guide", expanded=False):
        st.markdown("""*Disclaimer: ... * **N/A:** Data Not Available (typically due to API errors or insufficient history). ... **DYOR (Do Your Own Research).**""", unsafe_allow_html=True) # Condensed for brevity
    st.divider(); st.caption("Disclaimer: Informational/educational tool only. Not financial advice. DYOR.")
except Exception as main_exception: logger.exception("!!! UNHANDLED ERROR IN MAIN APP EXECUTION !!!"); st.error(f"An unexpected error occurred: {main_exception}. Please check the application log below for details.")
...
log_content = log_stream.getvalue(); st.text_area("Log:", value=log_content, height=300, key="log_display_area_v146", help="Select All (Ctrl+A or Cmd+A) and Copy (Ctrl+C or Cmd+C) to analyze or share.") # Unique key for log area
logger.info("--- End of Streamlit script execution (v1.4.6) ---"); log_stream.close()
